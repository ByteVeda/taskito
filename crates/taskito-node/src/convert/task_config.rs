...
/// silently disabling the cap (a misconfigured limit is a config error). Names
/// the option and the task, since several options share this grammar and the
/// error otherwise cannot say which config is wrong.
fn parse_rate_spec(field: &str, task: &str, spec: Option<&str>) -> Result<Option<RateLimitConfig>> {
    match spec {
...
pub fn task_config(input: &TaskConfigInput) -> Result<TaskConfig> {
    Ok(TaskConfig {
        ...
        rate_limit: parse_rate_spec("rateLimit", &input.name, input.rate_limit.as_deref())?,
        ...
        retry_budget: parse_rate_spec("retryBudget", &input.name, input.retry_budget.as_deref())?,
