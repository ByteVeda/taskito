...
const RESOURCE_COLUMNS: Column<ResourceStatus>[] = [
  ...
  { header: "Init (ms)", accessor: (r) => <span class="tabular-nums text-muted">{r.init_duration_ms.toFixed(1)}ms</span> },
  { header: "Recreations", accessor: (r) => <span class={`tabular-nums ${r.recreations > 0 ? "text-warning" : "text-muted"}`}>{r.recreations}</span> },
  {
    header: "Dependencies",
    accessor: (r) =>
      r.depends_on.length
    ...
    accessor: (r) =>
      r.pool ? (
        <span class="text-xs tabular-nums">
          <span class="text-info">{r.pool.active}</span>/{r.pool.size} active, <span class="text-muted">{r.pool.idle} idle</span>
        </span>
      ) : <span class="text-muted">{"\u2014"}</span>,
  },
];
...
export function Resources(_props: RoutableProps) {
  ...
  return (
    <div>
      ...
      {!resources?.length ? (
        <EmptyState message="No resources registered" subtitle="Resources appear when workers start with DI configuration" />
      ) : (
