...
class TestPassStrategy:
    def test_int_passes_through(self, strict):
        args, _kw = strict.intercept((42,), {})
        assert args == (42,)

    def test_str_passes_through(self, strict):
        args, _kw = strict.intercept(("hello",), {})
        assert args == ("hello",)

    def test_float_passes_through(self, strict):
        args, _kw = strict.intercept((3.14,), {})
        assert args == (3.14,)

    def test_bool_passes_through(self, strict):
        args, _kw = strict.intercept((True, False), {})
        assert args == (True, False)

    def test_none_passes_through(self, strict):
        args, _kw = strict.intercept((None,), {})
        assert args == (None,)

    def test_bytes_passes_through(self, strict):
        args, _kw = strict.intercept((b"data",), {})
        assert args == (b"data",)

    def test_mixed_primitives(self, strict):
...
class TestConvertStrategy:
    def test_uuid_round_trip(self, strict):
    ...
    def test_datetime_round_trip(self, strict):
    ...
    def test_date_round_trip(self, strict):
    ...
    def test_time_round_trip(self, strict):
    ...
    def test_timedelta_round_trip(self, strict):
    ...
    def test_decimal_round_trip(self, strict):
    ...
    def test_path_round_trip(self, strict):
    ...
    def test_pattern_round_trip(self, strict):
    ...
    def test_enum_converts(self, strict):
    ...
    def test_dataclass_converts(self, strict):
        @dataclass
        class Point:
            x: int
            y: int
    ...
    def test_datetime_before_date(self, strict):
...
class TestRedirectStrategy:
    def test_redirect_produces_marker(self, strict):
...
class TestRejectStrategy:
    def test_threading_lock_rejected(self, strict):
        lock = threading.Lock()
    ...
    def test_socket_rejected(self, strict):
    ...
    def test_generator_rejected(self, strict):
        gen = (x for x in range(10))
        with pytest.raises(InterceptionError):
            strict.intercept((gen,), {})

    def test_reject_error_includes_path(self, strict):
        lock = threading.Lock()
        with pytest.raises(InterceptionError) as exc_info:
            strict.intercept((), {"session": lock})
        assert "kwargs.session" in exc_info.value.failures[0].path

    def test_reject_error_has_suggestions(self, strict):
        lock = threading.Lock()
        with pytest.raises(InterceptionError) as exc_info:
            strict.intercept((lock,), {})
        assert len(exc_info.value.failures[0].suggestions) > 0

    def test_multiple_rejections_collected(self, strict):
        lock = threading.Lock()
...
class TestLenientMode:
    def test_rejected_arg_dropped(self, lenient):
        lock = threading.Lock()
        args, _kw = lenient.intercept((42, lock), {})
        assert args[0] == 42
        assert args[1] is None  # dropped to None

    def test_rejected_kwarg_dropped(self, lenient):
        lock = threading.Lock()
...
class TestOffMode:
    def test_passthrough_no_interception(self, registry):
...
class TestRecursiveWalking:
    ...
    def test_circular_reference_handled(self, strict):
        d: dict = {"value": 42}
        d["self"] = d  # circular!
...
class TestRoundTrip:
    def test_uuid_full_round_trip(self, strict):
    ...
    def test_mixed_types_round_trip(self, strict):
    ...
    def test_nested_convert_round_trip(self, strict):
        uid = uuid.uuid4()
...
class TestAnalyze:
    ...
    def test_analyze_on_off_mode_returns_empty(self, registry):
        interceptor = ArgumentInterceptor(registry, mode="off")
        report = interceptor.analyze((42,), {})
        assert len(report.entries) == 0

    def test_report_str_format(self, strict):
...
class TestQueueIntegration:
    def test_queue_default_interception_off(self, tmp_path):
        q = Queue(db_path=str(tmp_path / "test.db"))
        assert q._interceptor is None

    def test_queue_strict_mode(self, tmp_path):
    ...
    def test_queue_enqueue_with_interception(self, tmp_path):
        q = Queue(db_path=str(tmp_path / "test.db"), interception="strict")
    ...
    def test_queue_enqueue_rejects_lock(self, tmp_path):
        q = Queue(db_path=str(tmp_path / "test.db"), interception="strict")
    ...
    def test_task_analyze(self, tmp_path):
        q = Queue(db_path=str(tmp_path / "test.db"), interception="strict")

        @q.task()
        def my_task(user_id, created_at):
            pass

        report = my_task.analyze(42, datetime.datetime.now())
        assert len(report.entries) == 2

    def test_task_analyze_off_mode(self, tmp_path):
        q = Queue(db_path=str(tmp_path / "test.db"))
...
class TestCustomRegistration:
    def test_register_custom_reject(self, registry, strict):
        class MyLock:
            pass
    ...
    def test_register_custom_convert(self, registry, strict):
        class Money:
            def __init__(self, amount: int, currency: str):
                self.amount = amount
                self.currency = currency

        def convert_money(obj):
            return {
