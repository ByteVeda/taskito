...
Per-job lists (`delay_list`, `metadata_list`, `expires_list`, `result_ttl_list`) override uniform values when both are provided. See the [API reference](../api/queue.md#queueenqueue_many) for the full parameter list.
