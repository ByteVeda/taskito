...
def main() -> None:
    ...
    # 2. Cancel at enqueue: tenant not in allowlist + on_false='cancel'
    #    is implied by the Cancel return path of TenantIn... actually,
    #    TenantIn returns False which combines with on_false='defer'
    #    here, so the call is deferred. Use TenantIn(...).evaluate()
    #    behavior plus on_false='cancel' for hard rejection:
    try:
        send_daily_report.delay(tenant="initech")
    except PredicateRejectedError:
        print("not allowed")

    # 3. Bypass the billing flag by setting it in env.
    import os
    os.environ["FF_NEW_BILLING"] = "true"
    charge_card.delay(tenant="acme", cents=4200)

    # 4. Async predicate + async task — both awaited transparently.
...
* See the [Predicates guide](/docs/guides/core/predicates) for the
  full API surface.
