...
is <SdkSwap python={<code>CloudpickleSerializer</code>} node={<code>JsonSerializer</code>} java={<code>JsonSerializer</code>} />,
which <SdkSwap python="supports lambdas, closures, and complex Python objects" node="emits human-readable JSON bytes" java="emits human-readable JSON bytes via Jackson" />.
