...
<Mermaid
  chart={`flowchart TB
    subgraph dispatch["DISPATCH LAYER — enhanced poller"]
      d1["Local deque"] ~~~ d2["Affinity routing"] ~~~ d3["Work-stealing"]
    end
    subgraph mesh["MESH LAYER — taskito-mesh crate"]
      m1["SWIM gossip"] ~~~ m2["Membership"] ~~~ m3["Failure detection"]
    end
    subgraph durability["DURABILITY LAYER — unchanged"]
...
<Mermaid
  chart={`flowchart LR
    subgraph deque["VecDeque — front ← steal · pop → back"]
      direction LR
      j1["Non-owned"] --- j2["Non-owned"] --- j3["Mixed"] --- j4["Owned"] --- j5["Owned"]
    end
    steal["steal(n) FIFO"] -.-> j1
    j5 -.-> pop["pop() LIFO"]
  `}
/>
