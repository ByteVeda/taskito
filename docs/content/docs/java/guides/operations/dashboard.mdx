...
with the token stripped from the URL, so the SPA works for the rest of the
session without the secret leaking into browser history, `Referer` headers, or
access logs — a `?token=` query is never accepted on `/api/*` calls. OAuth has
