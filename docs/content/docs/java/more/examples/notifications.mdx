    ...
    public static void main(String[] args) throws InterruptedException {
        try (Taskito taskito = Taskito.builder().sqlite("notifications.db").open()) {
            ...
            try (Worker worker = taskito.worker()
                    .handle(Tasks.SEND_EMAIL, email -> emailProvider.send(email))
                    .handle(Tasks.SEND_SMS, sms -> smsProvider.send(sms))
                    .handle(Tasks.SEND_DIGEST, ignored -> buildAndFanOutDigests())
                    .queues("default")
                    .start()) {
