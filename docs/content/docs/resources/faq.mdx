...
- Worker threads are OS threads managed by Rust, not{" "}
  <SdkSwap python="Python processes" node="separate Node processes" />
- No external dependencies beyond{" "}
  <SdkSwap python={<><code>cloudpickle</code></>} node="the native addon" />

## Can I use async tasks?

Yes. Define the task function with{" "}
<SdkSwap python={<code>async def</code>} node={<code>async</code>} /> and the
...
<SdkSwap python={<code>current_job</code>} node={<code>currentJob()</code>} />{" "}
context in async tasks.

## What serialization format does taskito use?

By default, <SdkSwap python={<code>CloudpickleSerializer</code>} node={<code>JsonSerializer</code>} /> — <SdkSwap python="which supports most Python objects including lambdas and closures; switch to JsonSerializer for simpler, cross-language payloads" node="human-readable JSON bytes; switch to MsgpackSerializer for compact, binary payloads" /> — or provide a custom serializer:
