...
try (Worker worker = queue.worker()
        .handle(fetchUrls, urls -> urls.stream().map(App::fetch).toList())
        .concurrency(8)
        .start()) {
    String id = queue.enqueue(fetchUrls, urls);
    List<String> result = queue.awaitJob(id, Duration.ofSeconds(30))
        .flatMap(job -> queue.getResult(id, List.class))
        .orElseThrow();
}
```
