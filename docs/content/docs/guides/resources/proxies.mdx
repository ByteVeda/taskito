...
DI-injectable — things like file handles, HTTP sessions, and cloud clients
that have capturable state.
...
Use `NoProxy` when the serializer can handle the value directly (e.g.,
with cloudpickle) or when you want to suppress proxy handling for a
specific call without disabling the handler globally.
