...
<Mermaid
  chart={`graph LR
    ...
    clean --> load:::sink
    enrich --> load
    classDef source fill:#1f6feb,color:#fff
    classDef sink fill:#238636,color:#fff`}
/>
...
export function runPipeline(runId: string) {
  return queue.workflows
    ...
    .submit({ queueDefault: "default", params: { runId } });
}
```
...
export function report(runId: string) {
  ...
  if (node?.jobId) {
    for (const err of queue.getJobErrors(node.jobId)) {
