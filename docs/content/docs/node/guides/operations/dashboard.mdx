...
httpOnly cookie and redirects with the token stripped from the URL, so the SPA
works for the rest of the session without the secret leaking into browser
history, `Referer` headers, or access logs. The token is compared in constant
time. You can also mount the dashboard inside an existing app behind
