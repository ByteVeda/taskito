...
1. **Retry** — if attempts remain, the job is rescheduled with
   [backoff](/node/guides/reliability/retries).
2. **Dead-letter** — once the budget is exhausted, the job moves to the
   [dead-letter queue](/node/guides/reliability/dead-letter).
...
```ts
queue.getJobErrors(id); // one entry per failed attempt
queue.listJobs({ status: "failed" });
queue.deadLetters(); // exhausted jobs
```
