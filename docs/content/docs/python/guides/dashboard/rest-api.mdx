...
instant the snapshot was taken. Read-only — nothing is deleted. The dashboard
process previews its own windows, which may differ from a worker running
configured windows elsewhere.
