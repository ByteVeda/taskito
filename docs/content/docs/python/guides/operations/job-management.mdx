...
a human must act on. The scheduler checks every ~60 seconds and purges rows
older than their table's window.
...
Pass a fully empty `Retention()` to disable auto-cleanup entirely and keep
every table forever. This is deliberately different from *omitting*
