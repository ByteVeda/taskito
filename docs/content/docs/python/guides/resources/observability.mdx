...
<Callout type="info">
  `taskito reload` sends `SIGHUP` — it does not wait for the reload to
  complete. Check `taskito resources` or logs to confirm the reload
  succeeded.
</Callout>
...
| `taskito_resource_recreation_total` | Gauge | `resource` | Total recreation count (a Gauge, not a Counter, despite the `_total` suffix — it's set to the current count on each poll rather than incremented) |
