...
description: "worker_resource decorator, scopes (worker/task/thread/request), dependencies, teardown, health checks."
---
...
<Callout type="info" title="Why not just a module-level global?">
  ...
  (worker / task / thread / request), injects it by name, and tears it down on
...
# Request scope: a fresh instance for every task, torn down right after
@queue.worker_resource("audit_context", scope="request")
