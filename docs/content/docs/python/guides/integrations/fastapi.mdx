...
description: "TaskitoRouter — pre-built APIRouter with job status, SSE progress, dead letter management."
---

taskito provides a pre-built `APIRouter` for FastAPI with endpoints for
...
`TaskitoRouter` accepts options to control which routes are registered,
