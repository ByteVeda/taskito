...
| `retention` | `Retention \| None` | `None` | Per-table retention windows. Retention is **on by default** — leaving both `result_ttl` and `retention` unset applies the recommended windows (`archived_jobs`/`task_metrics`/`job_errors` 7d, `task_logs` 3d, `dead_letter` 30d). Pass an empty `Retention()` to disable auto-cleanup and keep every table forever. See [Retention & auto-cleanup](/python/guides/operations/job-management#retention--auto-cleanup). |
