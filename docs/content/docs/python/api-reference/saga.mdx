...
For the conceptual overview, see [Sagas](/docs/guides/workflows/sagas).
...
`WorkflowState.is_terminal()` returns `True` for `Compensated` and `CompensationFailed`. `WorkflowRun.wait()` unblocks on any of the four terminal states (`Completed`, `Failed`, `Cancelled`, `Compensated`, `CompensationFailed`).
...
Subscribe to these on the queue's event bus via `queue.on(event_type, callback)`:
...
If the parent step has its own explicit `compensates=…`, that wins — no propagation happens. See [Sagas](/docs/guides/workflows/sagas) for a worked example.
