...
`WorkflowState.is_terminal()` returns `True` for `Compensated` and `CompensationFailed`. `WorkflowRun.wait()` unblocks on any of the five terminal states (`Completed`, `Failed`, `Cancelled`, `Compensated`, `CompensationFailed`).
