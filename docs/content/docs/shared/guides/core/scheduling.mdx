...
These act on the catalog in storage, which is written when
<SdkLink to="guides/core/workers">`run_worker()`</SdkLink> starts — a producer
process that never starts a worker sees an empty list. `@queue.periodic()` only
*declares* a schedule, so removing the decorator does **not** delete its row:
the schedule keeps firing and enqueuing jobs for a task name no running worker
handles anymore. Retire one with `delete_periodic()` rather than by deleting
the decorator.
