...
By default a log message is stored only when a log subscription already
exists at publish time. **Declaring** a topic lifts that late-join boundary:
its publishes are retained even with zero subscribers, so a consumer that
subscribes later still reads the whole backlog. Declaring is idempotent.
