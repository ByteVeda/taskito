...
taskito uses a **token bucket** algorithm to limit how fast tasks execute.
Rate limits are per-task and persisted in SQLite.
...
<Callout type="info" title="Rate limit state is persisted">
  ...
  `rate_limits` SQLite table. This means rate limits survive worker restarts.
</Callout>
